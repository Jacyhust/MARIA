...
#ifdef COUNT_CC
        std::vector<int> cc_per_insertion;
#endif
...
#ifdef COUNT_CC
            cc_per_insertion.resize(max_elements_ + 1);
#endif
...
//                    if (candidate_id == 0) continue;
        ...
        template <bool has_deletions, bool collect_metrics = false>
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst>
            searchBaseLayerSTWithDist(tableint ep_id, const void* data_point, size_t ef, DISTFUNC<dist_t> distfunc) const {
            VisitedList* vl = visited_list_pool_->getFreeVisitedList();
            vl_type* visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            ...
            dist_t lowerBound;
            if (!has_deletions || !isMarkedDeleted(ep_id)) {
                dist_t dist = distfunc(data_point, getDataByInternalId(ep_id), dist_func_param_);
                lowerBound = dist;
                top_candidates.emplace(dist, ep_id);
                candidate_set.emplace(-dist, ep_id);
            }
            ...
            while (!candidate_set.empty()) {
                ...
                for (size_t j = 1; j <= size; j++) {
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        char* currObj1 = (getDataByInternalId(candidate_id));
                        dist_t dist = distfunc(data_point, currObj1, dist_func_param_);
        ...
        void getNeighborsByHeuristic2(
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> &top_candidates,
        const size_t M, labeltype label) {
        ...
        tableint mutuallyConnectNewElement(const void *data_point, tableint cur_c,
                                       std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> &top_candidates,
        int level, bool isUpdate, labeltype label) {
            size_t Mcurmax = level ? maxM_ : maxM0_;
            getNeighborsByHeuristic2(top_candidates, M_, label);
            ...
            for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
                ...
                if (!is_cur_c_present) {
                    if (sz_link_list_other < Mcurmax) {
                        ...
                        getNeighborsByHeuristic2(candidates, Mcurmax, label);
...
//                        continue;
                    ...
                    getNeighborsByHeuristic2(candidates, layer == 0 ? maxM0_ : maxM_, maxlevel_);
        ...
        void repairConnectionsForUpdate(const void *dataPoint, tableint entryPointInternalId, tableint dataPointInternalId, int dataPointLevel, int maxLevel) {
            ...
            for (int level = dataPointLevel; level >= 0; level--) {
                std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> topCandidates = searchBaseLayer(
                        currObj, dataPoint, level, maxlevel_);
                ...
                if (filteredTopCandidates.size() > 0) {
                    ...
                    currObj = mutuallyConnectNewElement(dataPoint, dataPointInternalId, filteredTopCandidates, level, true, maxlevel_);
                }
            }
        }
            ...
            if ((signed)currObj != -1) {
                ...
                for (int level = std::min(curlevel, maxlevelcopy); level >= 0; level--) {
                    ...
                    std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates = searchBaseLayer(
                            currObj, data_point, level, label);
                    if (epDeleted) {
#ifdef COUNT_CC
                        cc_per_insertion[label]++;
#endif
                    ...
                    currObj = mutuallyConnectNewElement(data_point, cur_c, top_candidates, level, false, label);
                }
        ...
        searchKnnWithDist(const void* query_data, size_t k, DISTFUNC<dist_t> distfunc) const {
            std::priority_queue<std::pair<dist_t, labeltype >> result;
            if (cur_element_count == 0) return result;

            tableint currObj = enterpoint_node_;
            dist_t curdist = distfunc(query_data, getDataByInternalId(enterpoint_node_), dist_func_param_);
            ...
            for (int level = maxlevel_; level > 0; level--) {
                ...
                while (changed) {
                    ...
                    for (int i = 0; i < size; i++) {
                        ...
                        if (cand < 0 || cand > max_elements_)
                            throw std::runtime_error("cand error");
                        dist_t d = distfunc(query_data, getDataByInternalId(cand), dist_func_param_);
            ...
            std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>, CompareByFirst> top_candidates;
            if (has_deletions_) {
                top_candidates = searchBaseLayerSTWithDist<true, true>(
                    currObj, query_data, std::max(ef_, k), distfunc);
            }
            else {
                top_candidates = searchBaseLayerSTWithDist<false, true>(
                    currObj, query_data, std::max(ef_, k), distfunc);
            }
        ...
        void test(Preprocess* prep)
        {
            ...
            for (int u = 0; u < prep->benchmark.N - 200; ++u) {
                ...
                cnt1 += size;
                tableint* datal = (tableint*)(data + 1);
                std::vector<int> nns(datal, datal + size);
                //f = f & isUnique(nns);
                if (f == 0) {
                    printf("***\n");
                }
                ...
                for (auto& v : nns) {
                    res += sqrt(fstdistfunc_(getDataByInternalId(v), getDataByInternalId(u), dist_func_param_));
                }
            }
            ...
            int N1 = 0;
            int benchsize = prep->benchmark.N - 200;
            for (int u = 0; u < max_elements_; ++u) {
                int* data = (int*)get_linklist0(u);
                size_t size = getListCount((linklistsizeint*)data);
                cnt += size;
                sqrMat += size * size;
                ...
                int tt = getExternalLabel(u);
                if (tt >= benchsize) continue;
                ...
                int j = 0;
                for (auto& v : nns) {
                    set1.insert((unsigned)getExternalLabel(v));
                    set2.insert((unsigned)(prep->benchmark.indice[tt + 200][j]));
                    ++j;
                }
            ...
            int N = max_elements_;
            float avgD = (float)cnt / N1;
            float derivation = (float)sqrMat / N1 - ((float)cnt / N1) * ((float)cnt / N1);
            derivation = sqrt(derivation);
            auto idx = file.rfind('.');
            std::string fname(file.begin(), file.begin() + idx);
            fname = fname + "_hnswInfo_ml.txt";
            //std::cout << file << std::endl << fname << std::endl;
            FILE* fp = nullptr;
